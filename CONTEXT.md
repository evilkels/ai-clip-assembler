...
The active default AI Harness is `pi_agent`, which drives the `pi` CLI to add
visual-interest scoring. The `local_qwen` Local AI Harness is retained in code
but postponed and disabled until the local-model path is ready.
