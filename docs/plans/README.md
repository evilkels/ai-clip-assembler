...
| [016](016-edit-creation-clip-selection.md) | Fix edit creation — no duplicate clips, no unusable slivers, agent-influenced selection | P1 | M | 005, 012, agent-operable-timeline | STEP 1 DONE (2026-07-20) — draft-time overlap dedup + sliver floor in `build_draft_timeline` (12 duplicate clips → 2 distinct, no 1s tail); 362 backend tests + ruff green. Steps 2–4 (source-level candidate de-overlap, Review controls, agent-influenced selection) TODO. |
...
| [018](018-diverse-clip-generation.md) | Diverse clip generation (one fragment per moment, look-group variety, multi-format edits, controls→Import) | P1 | L | 012, 016, 017 | TODO (2026-07-20) — phased TDD plan on branch `feature/diverse-clip-generation`. Phase A: one best fragment per smooth run (**supersedes plan 016 Step 2**). Phase B: local ONNX CLIP embeddings + cosine look-group clustering (degrades if model absent). Phase C: diversity-aware Short/Medium/Long edits + sparing slow-mo. Phase D: library grouped by look. Phase E: relocate generation knobs to Import under Analyze. Maintainer decision flagged: exact ONNX CLIP model + redistribution license. |
