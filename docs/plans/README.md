...
| [018](018-diverse-clip-generation.md) | Diverse clip generation (one fragment per moment, look-group variety, multi-format edits, controls→Import) | P1 | L | 012, 016, 017 | IN PROGRESS (2026-07-20) — Phase A DONE (`edd4f16`, one best fragment per smooth run; supersedes plan 016 Step 2) and Phase E DONE (`2f48e2f`, `b0c46dc`, generation controls on Import with cached re-derive; focused Playwright + tsc + eslint green). Next: Phase B local ONNX CLIP embeddings + cosine look-group clustering, then C/D. Maintainer decision remains: exact ONNX CLIP model + redistribution license. |
