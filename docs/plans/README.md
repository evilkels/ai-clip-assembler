...
| [project-sidebar](project-sidebar.md) | Partially implemented; auto-reopen persistence shipped. Remaining deferred pending visual QA: `Cmd-B` collapse/resizable rail, context menus, and keyboard-navigation verification. |
| [settings-page](settings-page.md) | **Partially implemented — do NOT move to `done/`** (2026-07-02 audit). Shipped: `SettingsModal.tsx` with theme, Diagnostics, Connect-your-AI, and a 4-field harness form backed by `GET/PUT /settings`. Unbuilt: 4 of 5 setting sections (scoring, storage, export, app incl. ffmpeg-path override) and the entire per-project override model (deep-merge, `getEffective`/`setOverride` IPC, inherit/override UI) — `project.json.settings_overrides` exists but nothing reads it as settings. Override model needs a design pass before implementation. |
