...
| [017](017-review-page-clarity-and-polish.md) | Review-page clarity & polish | P1 | M | 012, 016, ui-polish-modern-shell | STEP 1 DONE (2026-07-20) — reordered Review (Suggested cuts → Browse your clips → Advanced knobs), reframed generation panel + copy, made clips discoverable, and made "exclude" actually drop clips from the AI candidate pool (`_review_inputs` filter + test); 363 backend tests + tsc + eslint green. Steps 2–6 (poster thumbnails/open-by-default, consolidate smoothness controls, honor "included" preference, mental-model onboarding, design-system adoption) TODO. |
| [018](018-diverse-clip-generation.md) | Diverse clip generation (one fragment per moment, look-group variety, multi-format edits, controls→Import) | P1 | L | 012, 016, 017 | IN PROGRESS (2026-07-21) — Phases A, B, C, E DONE. A (`edd4f16`) one best fragment per smooth run (supersedes plan 016 Step 2); E (`2f48e2f`, `b0c46dc`) generation controls on Import with cached re-derive; B (`c319374`…`6582af6`) embedding provider, cosine look-group clustering, persisted embeddings + `look_group`; C (`738b5e6`, `c0c5e43`) one clip per look group, Short/Medium/Long formats, sparing slow-mo, format-aware draft endpoint. 393 backend tests + ruff green. Model decision settled: SigLIP base-patch16-224 (Apache-2.0) over CLIP/MobileCLIP. D (`c0f521b`) look-group card grouping with "N similar looks" expander plus a Short/Medium/Long switcher on Suggested cuts; tsc + eslint clean. Phase D's RTL test was deliberately skipped — this repo has no renderer component-test framework (only Playwright e2e + `node --test` for the main process); maintainer decision 2026-07-21. CODE COMPLETE. Remaining, not code: export and bundle `backend/models/siglip_image_encoder.onnx` — until it ships `default_embedding_provider()` returns `None`, every clip is its own look group, and the whole diversity feature is inert in real runs. |
