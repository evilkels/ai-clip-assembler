...
| [agent-operable-timeline](agent-operable-timeline.md) | **Phase A1 landed** (backend `TimelineDocument` + reversible operations core + snapshot undo/redo + write lock + persistence/migration; 56 tests). **At the A1 review gate — awaiting human review before A2.** Remaining: A2 (HTTP+SSE+GUI), B (MCP), C (in-app agent). Spec: `docs/specs/2026-06-19-agent-operable-timeline-design.md`. |
