...
transform/split/remove + undo/redo). Plan 020 closes the remaining visual
...
Plan 021 closes the Export projection defect: Export reads the current backend
Timeline Document without a legacy pre-export write, preserves the complete
backend result and warnings per format, and exposes every ordered item in its
payload review. **Remaining**: full
...
covered by the Plan 020 browser regression. The Plan 021 browser regression
covers repeated-item export, item-derived effective duration, EDL degradation
warnings, and the absence of a legacy Timeline write. Backend tests, frontend
checks, and synthetic E2E remain automated evidence; real-footage and NLE import
validation remain manual QA.

## Why this matters / decisions locked (see spec for rationale)

Before the authoritative Operations core, the timeline was GUI-only and thin,
owned by the frontend `ReviewContext` and unable to express real edits (split,
extend, speed, transform, multi-instance). Goal, per Palmier Pro's principle (*the editor is
