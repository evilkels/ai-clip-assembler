...
- **C — Portability**: copy the whole project folder to another volume; DaVinci must still resolve media with zero relink prompts. Failure here means export paths are absolute, not relative — or, as [plan 032](032-valid-fcpxml-and-nle-verification.md) suspects, relative but not written as URLs, which fails even without moving the folder.
