...
1. **Backend: project loader/writer** — `project.json` read/write, folder-scan, schema validation, idempotent create. Pure Python, unit-tested.
2. **Backend: relocate analysis output** — move whatever the current pipeline writes into `clipassembler/samples/` and `clipassembler/analysis/`.
3. **Backend: exports write inside project folder** — change export endpoints to take a project, not arbitrary paths.
4. **Frontend: Create / Open Project flow** — Electron folder picker, `recent.json` in app-data, sidebar entry.
5. **Frontend: project-scoped views** — current drone-video workflow becomes "the open project's view".
6. **Docs: extend `MANUAL_QA_GUIDE.md`** with the three flows above.
